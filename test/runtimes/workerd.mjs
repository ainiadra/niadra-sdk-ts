...
// with the webhook adapters and @niadra/sdk/cloudflare-agents (bundled with the AI SDK the way
// Wrangler bundles a Worker; a Node-only import fails the bundle).
...
const adapters = await Promise.all(
  ["elevenlabs", "twilio", "whatsapp", "vapi", "retell"].map(async (name) => ({
    type: "ESModule",
    path: `${name}.js`,
...
const bundled = await build({
  entryPoints: [new URL("agents.mjs", here).pathname],
  bundle: true,
  write: false,
  format: "esm",
  platform: "browser",
...
const agents = bundled.outputFiles[0].text;
const worker = `
...
export default {
  async fetch() {
    try {
      return new Response("ok workerd: " + (await smoke()) + " requests, " + (await webhooks()) + " webhook checks, " + (await agents()) + " agent checks");
    } catch (error) {
...
const mf = new Miniflare({
  modules: [
    ...
    { type: "ESModule", path: "agents.mjs", contents: agents },
    ...adapters,
  ],
