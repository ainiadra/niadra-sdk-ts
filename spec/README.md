...
still answer. `test/spec.test.ts` keeps the SDK's `ContextPack`, `PackSection`, `PackStamp` and
`PackSlot` types equal to the v1 schema, field by field, and reads both examples. When the
